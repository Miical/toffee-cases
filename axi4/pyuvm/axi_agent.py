...
class AXISeqItem(uvm_sequence_item):
    ...
    def __init__(self, name="AXI4SeqItem", tr_type=AXISeqItemType.READ, addr=0, len=0, data=[]):
        super().__init__(name)
...
class AXIDriver(uvm_driver):
    ...
    async def drive_a_pkt(self, seq_item):
        if seq_item.tr_type == AXISeqItemType.READ:
            await self.ar_send_addr(seq_item.addr, seq_item.len-1, 3, 1)
        ...
        elif seq_item.tr_type == AXISeqItemType.WRITE:
            await self.aw_send_addr(seq_item.addr, seq_item.len-1, 3, 1)
...
class AXIMonitor(uvm_monitor):
    ...
    async def collect_aw_w_pkt(self):
        while True:
            seq_item = AXISeqItem(tr_type=AXISeqItemType.WRITE)
            seq_item.data = []
            seq_item.len = 0
            while True:
    ...
    async def collect_b_pkt(self):
        while True:
            if self.dut.io_in_b_valid.value and self.dut.io_in_b_ready.value:
                seq_item = AXISeqItem(tr_type=AXISeqItemType.WRITE_RESP)
                self.ap.write(seq_item)
            await RisingEdge(self.dut.clock)

    async def collect_r_pkt(self):
        while True:
            seq_item = AXISeqItem(tr_type=AXISeqItemType.READ_RESP)
            seq_item.data = []
            seq_item.len = 0
